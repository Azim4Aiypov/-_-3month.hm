...
def main(page: ft.Page):
    ...
    def get_greeting(name):
         ...
         if 6 <= current_hour < 12:
            return f"Доброе утро, {name}!", ft.colors.YELLOW_600
         elif 12 <= current_hour < 18:
            return f"Добрый день, {name}!", ft.colors.ORANGE_600 
         elif 18 <= current_hour < 24:
            return f"Добрый вечер, {name}!", ft.colors.RED_600  
         else:
            return f"Доброй ночи, {name}!", ft.colors.BLUE_600

    def on_button_click(e):
        name = name_input.value.strip()

        if name:
            greeting_text.value, greeting_text.color = get_greeting(name)
